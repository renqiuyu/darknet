...
typedef struct PRIndicator PRIndicator;
...
void printPR(PRIndicator pr) {
	printf("with thresh : %.2f, precision : %.2f, recall : %.2f, mAP: %.2f, avg_iou:%.2f, f1_score: %.2f", 
		pr._thresh, pr._precision, pr._recall, pr._map, pr._average_iou, pr._f1_score);
}
void printPR_file(PRIndicator pr, FILE *fout) {
	fprintf(fout, "%.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n",
	 pr._thresh, pr._precision, pr._recall, pr._map, pr._average_iou, pr._f1_score);
}


PRIndicator validate_detector_map_pr(network net, char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou)
{
	int j;
	list *options = read_data_cfg(datacfg);
	char *valid_images = option_find_str(options, "valid", "data/train.txt");
	...
	// fprintf(stderr, "Total Detection Time: %f Seconds\n", (double)(time(0) - start));
	if (reinforcement_fd != NULL) fclose(reinforcement_fd);
	// 	Indicator(float thresh, float map, float precision, float recall, float f1_score, float average_iou, float fps) : 
	PRIndicator ret = {thresh_calc_avg_iou, mean_average_precision, cur_precision, cur_recall, f1_score, avg_iou, .0};
	return ret;
}


void validate_detector_pr(char *datacfg, char *cfgfile, char *weightfile, char *name, float prstep){
	network net = parse_network_cfg_custom(cfgfile, 1);	// set batch=1
	if (weightfile) {
		load_weights(&net, weightfile);
	}
	...
	char filename[256];
	if(name) {
		sprintf(filename, "csv/%s.csv", name);
		fout = fopen(filename, "a");
	} else {
		fout = fopen("csv/sample.csv", "a");
	}
	if(fout == 0) {
		fprintf(stderr, "%s\n", "fail to write csv file!");
	}
	fprintf(fout, "thresh, precision, recall, map, avg_iou, f1_score\n");
	for(float thresh = .0; thresh < 1.000001; thresh += prstep){
		PRIndicator temp = validate_detector_map_pr(net, datacfg, cfgfile, weightfile, thresh);
		printPR(temp);
		printPR_file(temp, fout);
	}
...
void run_detector(int argc, char **argv)
{
    ...
    char *csvname = find_char_arg(argc, argv, "-name", 0);
    float thresh = find_float_arg(argc, argv, "-thresh", .25);	// 0.24
	float hier_thresh = find_float_arg(argc, argv, "-hier", .5);
	float prstep = find_float_arg(argc, argv, "-step", .1); // default pr step
    ...
    else if(0==strcmp(argv[2], "prplot")) validate_detector_pr(datacfg, cfg, weights, csvname, prstep);
